...
class Save():
    def __init__(self):
        BASE_DIR = './'
        self.f_cookie = os.path.join(BASE_DIR, 'cookie.json')
        self.f_user_info = os.path.join(BASE_DIR, 'user_info.json')
    
    def get_cookies(self, url, user_name, user_pass, host):
        """
        获取cookie
        """
        payload = {
    ...
    def get_user_info(self, admin_cookie):
        """
        获取有效的公司
        :param admin_cookie:
        :return:
    ...
    def get_admin_cookie(self):
        ...
        with open(self.f_cookie, 'a+', encoding='utf-8') as fa:
            cookie_data = {}
            fa.seek(0)
            if os.path.getsize(self.f_cookie) != 0:
    ...
    def save_user_info(self, company_code):
        with open(self.f_user_info, 'w', encoding='utf-8') as fw:
            admin_cookie = self.get_admin_cookie()
            all_company_info = self.get_user_info(admin_cookie)
    ...
    def login_save_cookie(self, company_code):
        with open(self.f_user_info, 'a+', encoding='utf-8') as fr, \
                open(self.f_cookie, 'a+', encoding='utf-8') as fw:
            try:
                fr.seek(0)
            ...
            if not user_info:
                user_info = self.save_user_info(str(company_code))
    
            if user_info[0] == 'admin':
                cookie = self.get_admin_cookie()
            else:
